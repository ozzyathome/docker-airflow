...
class FileSensor1(BaseSensorOperator):
    """
    ...
    def poke(self, context):
        ...
        for path in glob(full_path):
            if os.path.isfile(path):
                self.log.info('File sensor 1 Found file %s', path)
                context['task_instance'].xcom_push('file_name', path)
                return True
...
class OsbPlugin(AirflowPlugin):
    name = "osb_plugin"
    operators = [FileSensor1]